...
class GroqChat(LLMBase):
    """Groq chat adapter using OpenAI-compatible API."""

    def __init__(self, api_key: str | None = None, base_url: str | None = None) -> None:
        try:
            from openai import AsyncOpenAI  # type: ignore
        except Exception as exc:  # noqa: BLE001
            raise RuntimeError("openai package is required for GroqChat. Install it to use this provider.") from exc
    ...
    def _build_chat_kwargs(self, messages: list[dict[str, Any]], config: CompletionConfig, stream: bool = False) -> dict[str, Any]:
        """Build kwargs for Groq chat completion."""
        kwargs: dict[str, Any] = {
            "model": config.model,
            "messages": messages,
        }
