...
class MistralChat(LLMBase):
    """Mistral AI chat adapter using OpenAI-compatible API."""

    def __init__(self, api_key: str | None = None, base_url: str | None = None) -> None:
        try:
            from openai import AsyncOpenAI  # type: ignore
        except Exception as exc:  # noqa: BLE001
            raise RuntimeError("openai package is required for MistralChat. Install it to use this provider.") from exc
    ...
    def _build_chat_kwargs(self, messages: list[dict[str, Any]], config: CompletionConfig, stream: bool = False) -> dict[str, Any]:
        """Build kwargs for Mistral chat completion."""
        kwargs: dict[str, Any] = {
            "model": config.model,
            "messages": messages,
        }
    ...
    async def complete(self, prompt: str, config: CompletionConfig) -> str:
        """Complete a prompt using Mistral."""
    ...
    def list_models(self) -> list[str]:
        """List available Mistral models (dynamically fetched)."""
        import json
